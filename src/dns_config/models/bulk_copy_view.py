...
class BulkCopyView(BaseModel):
  ...
  # noqa: E501
    ...
    __properties: ClassVar[List[str]] = [
        "auth_zone_config", "forward_zone_config", "recursive", "resources",
        "secondary_zone_config", "skip_on_error", "target"
    ]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of auth_zone_config
        if self.auth_zone_config:
            _dict['auth_zone_config'] = self.auth_zone_config.to_dict()
        # override the default output from pydantic by calling `to_dict()` of forward_zone_config
        if self.forward_zone_config:
            _dict['forward_zone_config'] = self.forward_zone_config.to_dict()
        # override the default output from pydantic by calling `to_dict()` of secondary_zone_config
        if self.secondary_zone_config:
            _dict[
                'secondary_zone_config'] = self.secondary_zone_config.to_dict(
                )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "auth_zone_config":
            AuthZoneConfig.from_dict(obj["auth_zone_config"])
            if obj.get("auth_zone_config") is not None else None,
            "forward_zone_config":
            ForwardZoneConfig.from_dict(obj["forward_zone_config"])
            if obj.get("forward_zone_config") is not None else None,
            "recursive":
            obj.get("recursive"),
            "resources":
            obj.get("resources"),
            "secondary_zone_config":
            AuthZoneConfig.from_dict(obj["secondary_zone_config"])
            if obj.get("secondary_zone_config") is not None else None,
            "skip_on_error":
